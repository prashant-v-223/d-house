...
const PartnersTable = () => {
  ...
  const profitDetailsApi = (id) => {
    const apiUrl =`https://doller-production-0ac2.up.railway.app/reward/get?userId=${id}`;
    fetch(apiUrl)
      .then((response) => {
        if (!response.ok) {
  ...
  const profitDetailsApi1112 = async (id) => {
    let response = await fetch(`https://doller-production-0ac2.up.railway.app/team/leval5-membe22/${id}`, {
      method: "GET",
    });
